...
class BCQNetwork:
	def __init__(self, name, state_dim, action_dim, max_action, actor_hs_list, actor_lr, critic_hs_list, critic_lr, dqda_clipping=None, clip_norm=False):
		...
		with tf.compat.v1.variable_scope(self.name):
			...
			self.state_ = tf.compat.v1.placeholder(tf.float32, [None, state_dim], name='state')
			# placeholders for actor network
			self.action_ = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, action_dim], name='action')
			# placeholders for target critic soft q network
			self.reward_ = tf.compat.v1.placeholder(tf.float32, [None], name='rewards')
			self.discount_ = tf.compat.v1.placeholder(tf.float32, [None], name='discounts')
			self.flipped_done_ = tf.compat.v1.placeholder(tf.float32, [None], name='flipped_dones')
			# placeholders for training critic network
			self.target_ = tf.compat.v1.placeholder(tf.float32, [None, 1], name='target')
			...
			self.actor_optim_ = tf.compat.v1.train.AdamOptimizer(learning_rate=actor_lr).minimize(self.actor_loss_)
			...
			self.critic_loss_ = tf.compat.v1.losses.mean_squared_error(self.target_, self.critic_1_out_) \
								+ tf.compat.v1.losses.mean_squared_error(self.target_, self.critic_2_out_)
			self.critic_optim_ = tf.compat.v1.train.AdamOptimizer(learning_rate=critic_lr).minimize(self.critic_loss_)
...
class VAE(object):
	def __init__(self, state_dim, action_dim, latent_dim, max_action, vae_lr):
		...
		self.epsilon_ = tf.random.normal([self.random_batch_size_, latent_dim], mean= 0.0, stddev=1.0)
		...
		self.recon_loss_ = tf.compat.v1.losses.mean_squared_error(self.d3_, self.action_)
		self.KL_loss_ = -0.5 * tf.reduce_mean((1. + tf.math.log(tf.pow(self.std_,2)) - tf.pow(self.mean_,2)  - tf.pow(self.std_,2)))
		self.loss_ = self.recon_loss_ + 0.5 * self.KL_loss_
		self.optim_ = tf.compat.v1.train.AdamOptimizer(learning_rate=vae_lr).minimize(self.loss_)
